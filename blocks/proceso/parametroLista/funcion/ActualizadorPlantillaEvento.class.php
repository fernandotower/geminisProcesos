...
class RegistradorEvento {
	...
	function procesarFormulario() {
		...
		$datos['intervalo']=$this->armarIntervaloTiempo();
		...
		$resultadoPlantillaEvento = $miComponente->actualizarEventoPlantilla ( $datosJson );
		
		//Este if para el caso del primer evento que no tiene relaciones modificables
		if ($resultadoPlantillaEvento==TRUE) {
			$this->miConfigurador->setVariableConfiguracion ( 'mostrarMensaje', 'eventoPlantillaActualizado' );;
		}
		
		if ($resultadoPlantillaEvento == TRUE && isset($_REQUEST['evento_precedente'])) {
			
			// Consultar relacion de los eventos en la tabla plantillaevento_orden
		   
			$datos ['id_plantillaevento_orden'] =$_REQUEST['id_plantillaevento_orden'];
			$datos ['id_evento1'] =$_REQUEST['evento_precedente'];				
			$datos ['id_evento2'] = $datos ['id_plantillaevento'];//el evento que se está editando, no cambia			
			$intervaloTiempo = $this->armarIntervaloTiempo ();
			$datos ['intervalo'] = $intervaloTiempo;			
			$datosRelacionJson=json_encode($datos);
			
			$resultadoRelacion=$miComponente->actualizarRelacionEventos($datosRelacionJson);
				
			
			if ($resultadoRelacion == TRUE) {
				$this->miConfigurador->setVariableConfiguracion ( 'mostrarMensaje', 'eventoPlantillaActualizado' );
			} else {
				$this->miConfigurador->setVariableConfiguracion ( 'mostrarMensaje', 'errorEventoPlantillaActualizado' );
			}
	...
	function armarIntervaloTiempo() {
		...
		if (isset ( $_REQUEST ['evento_precedente'] )) {
			...
			if (isset ( $_REQUEST ['minutos'] ) and $_REQUEST ['minutos'] !== "" and $_REQUEST ['minutos'] !== "0") {
				$intervaloTiempo .= $_REQUEST ['minutos'] . "M";
			}
